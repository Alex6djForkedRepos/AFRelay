...
from tests.test_wsfev1.mocks.soap_responses import FECompTotXRequestResponse
...
async def test_fe_comp_tot_x_request(client: AsyncClient, wsfe_httpserver_fixed_port, wsfe_manager, override_auth):
    ...
    resp = await client.post("/wsfev1/FECompTotXRequest", json=payload)
...
@pytest.mark.asyncio
async def test_fe_comp_tot_x_error(
                                        client: AsyncClient, 

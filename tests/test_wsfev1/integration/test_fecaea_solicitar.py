...
from tests.test_wsfev1.mocks.soap_responses import FECAEASolicitarResponse
...
async def test_fecaea_solicitar_success(client: AsyncClient, wsfe_httpserver_fixed_port, wsfe_manager, override_auth):
    ...
    resp = await client.post("/wsfev1/FECAEASolicitar", json=payload)
...
@pytest.mark.asyncio
async def test_fecaea_solicitar_error(
                                        client: AsyncClient, 
